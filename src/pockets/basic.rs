...
impl<E: Pairing> BasicParameters<E> {
    pub fn new<R: RngCore>(max_degree: usize, rng: &mut R) -> Result<Self, PocketError> {
        ...
        let powers_of_g1 = (0..max_degree + 1)
            .map(|i| {
                if i != 0 {
                    cur = cur.mul(&tau);
                }
...
impl<E: Pairing> BasicPolyCommit<E> {
    pub fn commit(
        ...
        let commit = multiexp::<E>(
            &params.g1_vec(poly_degree + 1).unwrap(),
            poly.deref().to_vec(),
        )
        .unwrap();
        Ok(Self { commit })
    ...
    pub fn open(
        ...
        params: &BasicParameters<E>,
        poly: &UniPolynomial<E>,
    ) -> Result<UniPolynomial<E>, PocketError> {
...
impl<E: Pairing> BasicProof<E> {
    pub fn create_witness(
        ...
        let w = multiexp::<E>(
            &params.g1_vec(res_poly.degree() + 1).unwrap(),
    ...
    pub fn verify_eval(
        ...
        let rhs = E::multi_pairing(
            ...
            [
                &(params.g2_tau.into_group() - params.g2.mul(point).into()).into(),
                &params.g2,
            ],
        );
...
#[cfg(test)]
mod basic {
    use super::{BasicParameters, BasicPolyCommit, BasicProof, UniPolynomial};
