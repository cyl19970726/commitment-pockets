...
use ark_ff::{
    BigInt, BigInteger, BigInteger128, BigInteger320, BigInteger64, CyclotomicMultSubgroup, Field,
    One, PrimeField,
};
...
pub fn pippenger<E: Pairing>(
    ...
    // k_0,0 k_1,0 k_2,0  window_index_set_vec[0]
    // k_0,1 k_1,1 k_2,1 window_index_set_vec[1]
    // k_0,2 k_1,2 k_2,2 window_index_set_vec[2]
    ...
    let result_g = window_index_set_vec.into_iter().enumerate().fold(
        E::G1::zero().into(),
        |acc_B, (j, window_value)| {
            // change to array
            // the max number for lambda is 2^N_Bit -1 which equal to the window size
            let mut lambda_to_P: [Vec<usize>; 2_usize.pow(N_BIT as u32)] =
                std::array::from_fn(|_| Vec::new());

            // we collect different P_i with the same lambda into a Vector 'lambda_to_P[lambda]'
            // if the j equal to 0, the window_value is the first window which consist of [k_0,0 k_1,0 k_2,0] the first index is i and the second index is j
            window_value.into_iter().enumerate().for_each(
                // lambda express the coefficient for each P_i
                |(i, lambda)| {
                    if lambda != 0 {
                        lambda_to_P[lambda as usize].push(i);
                    }
                },
            );

            // Calculate T Set
            let mut T: [E::G1Affine; 2_usize.pow(N_BIT as u32)] =
                std::array::from_fn(|_| E::G1::zero().into());
            for index in 0..lambda_to_P.len() {
                let s_index = lambda_to_P.len() - 1 - index;
                if lambda_to_P[s_index].len() == 0 {
                    T[index] = E::G1::zero().into();
                } else {
                    T[index] = lambda_to_P[s_index]
                        .iter()
                ...
                if index != 0 {
                    T[index] = T[index].add(T[index - 1].clone()).into();
                }
            }
            ...
            let B_j = T
                .into_iter()
                .enumerate()
                .fold(E::G1::zero().into(), |acc, (lambda, acc_value)| {
                    acc.add(acc_value).into()
                });

            let mut powers = <<E as Pairing>::ScalarField as PrimeField>::BigInt::from(1_u64);
            powers.muln(N_BIT as u32 * j as u32);
            (acc_B + B_j.mul(E::ScalarField::from_bigint(powers).unwrap())).into()
        },
    );
...
pub fn Combine_nbit_form<F: PrimeField>(window_values: Vec<u32>, nbit: usize) -> F {
    ...
    window_values
        .into_iter()
        .for_each(|v| res.push(BigInteger128::from(v)));
...
mod tests {
    ...
    fn test_pippenger() {
        let rng: &mut rand::rngs::ThreadRng = &mut thread_rng();
        let poly_degree = 4;
        let params = BasicParameters::<Bls12_381>::new(poly_degree + 1, rng).unwrap();
        ...
        let v1 = multiexp::<Bls12_381>(&params.clone().g1_vec(poly_degree + 1).unwrap(), poly_vec1)
            .unwrap();
        let v2 =
            pippenger::<Bls12_381>(&params.g1_vec(poly_degree + 1).unwrap(), poly_vec2).unwrap();
        assert_eq!(v1, v2);
    }
    ...
    fn test_vec_map() {
        let a = vec![1, 2, 3];
        let b = a.clone().into_iter().map(|x| x + 3);
        // println!("{}",b);
    }
}
